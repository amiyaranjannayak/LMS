...
def index(request):
    if request.user.is_superuser:
        ...
        new = Member.objects.filter(is_approve=0).count()
        pending = Member.objects.filter(is_approve=0).count()
        complete = Member.objects.filter(is_approve=1).count()
        reject = Member.objects.filter(is_approve=2).count()
        return render(request, 'index.html',{'admin_user': request.user,'new_orders': new,'pending_orders': pending,'complete_orders': complete,'reject_orders': reject})
    else:
        total = Member.objects.filter(user_id=request.user.id).count()
        complete = Member.objects.filter(is_approve=1,user_id=request.user.id).count()
        reject = Member.objects.filter(is_approve=2,user_id=request.user.id).count()
        recent_request = Member.objects.filter(is_approve=0,user_id=request.user.id).order_by('-id')
        return render(request, 'user_dashboard.html',{'normal_user': request.user,'total_requests': total,'complete_requests': complete,'reject_requests': reject, 'recent_requests': recent_request})

@login_required
def list(request):
    user_id = int(request.user.id)
    members_list = Member.objects.filter(user_id=user_id)
...
def lists(request):
    ...
    try:
        ...
        members = paginator.page(paginator.num_pages)
    return render(request, 'lists.html', {'members': members})
...
def create(request):
    if request.method == 'POST':
        ...
        try:
            ...
            pass
        member.save()
        messages.success(request, 'Request was created successfully!')
...
def delete(request, id):
    member = Member.objects.get(id=id)
    member.delete()
    messages.warning(request, 'Request deleted successfully!')
    return redirect('/list')
...
def updatePassword(request, id):
    if request.method == 'POST':
        ...
        member.password = request.POST['repeat_password']

        if current_password != new_password:
            messages.success(request, 'Password updated! Please login again.')
            return redirect('/change_password')
        else:
            member.set_password(request.POST['repeat_password'])
            try:
                member.full_clean()
            except ValidationError as e:
                pass
